...
{foreach name=list from=$paginator item=record}
    <td>
      <a href="/video/item/id/{$record['film_id']}"><img alt="{$record['name']}" title="{$record['name']}" src="/resources/posters/{$record['poster']}"></a>
      <h3><a href="/video/item/id/{$record['film_id']}">{$record['name']}{if $record['original_name'] != ""} / {$record['original_name']}{/if}</a></h3>
    </td>
